...
async function getAuthUser(req, res, next) {
    ...
    try {
        req.authUser = await localAuth.get(username);
        req.auth = localAuth;
        return next();
    } catch (err) {
...
module.exports = async function(paths) {
    ...
    const userPath = `${paths.users}/:username`;
    authRouter.put(
        userPath,
        authStatus(AuthLevels.ADMIN),
        bodyParser.json(),
        async(req, res) => {
            ...
            const { password, auth, replace } = body;

            let status;
            let message;
            try {
                await localAuth.create(username, password, auth, replace);
    ...
    authRouter.get(
        ...
        getAuthUser,
        async(req, res) => {
            const { password, authLevel } = req.authUser;
            res.send({ password, authLevel }).end();
        }
    );
    authRouter.delete(
        userPath,
