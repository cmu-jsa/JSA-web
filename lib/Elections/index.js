...
class Elections {
    ...
    /**
     * Gets a map of all election IDs to an object containing their `title`,
     * `candidates`, and `closed`.
     *
     * @param {string} [cookie] - If specified, each mapped object will
     * additionally contain a `voted` field, marking if the cookie was used.
    ...
    enumerate(cookie) {
        ...
        Object.keys(elections).forEach(key => {
            ...
            const { title, candidates, closed } = election;

            const result = { title, candidates, closed };
            cookie && (result.voted = election.voted(cookie));
            map[key] = result;
        });
        return map;
    }
